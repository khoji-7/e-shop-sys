            ...
            <thead className="bg-gray-100 border-2 border-gray-300">
              <tr>
                {[
                  ...
                  <th
                    key={header}
                    className="py-2 px-5 text-gray-600 font-medium"
                  >
                    {header}
                  </th>
                ))}
            ...
            <tbody>
              {paymentData?.length > 0 ? (
                paymentData.map((user) => (
                  <tr
                    key={user?.id}
                    className="border-2 border-gray-200 h-16 text-gray-500"
                  >
                    ...
                    <td className="px-5 py-1">{user?.zone || "Noma'lum"}</td>
                    <td className="px-5 py-1">{user?.cost || "Noma'lum"}</td>
                    <td className="px-5 py-1">
                      {user?.given_day
                        ? new Date(user.given_day).toLocaleString("en-GB")
                        : "Noma'lum"}
                    </td>
