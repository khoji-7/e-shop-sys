...
const UserModal = ({ user, closeModal }) => {
  ...
  const paymentStatusColor = user.payment_status
    ? "text-green-500"
    : "text-red-500";
          ...
          <table className="min-w-full table-auto border-collapse shadow-lg rounded-lg overflow-hidden">
            <tbody>
              ...
              <tr className="bg-gray-100">
                ...
                <td className="py-2 px-6 text-gray-900">
                  {Number(user.cost).toLocaleString()} so'm
                </td>
              </tr>
              <tr>
                <td className="py-2 px-6 text-gray-700">Oylik to'lov</td>
                <td className="py-2 px-6 text-gray-900">
                  {Number(user.monthly_income).toLocaleString()} so'm
                </td>
              </tr>
              ...
              <tr className="bg-gray-100">
                <td className="py-2 px-6 text-gray-700">Malumot</td>
                <td className="py-2 px-6 text-gray-900">{user.description}</td>
              </tr>
              <tr>
                <td className="py-2 px-6 text-gray-700">
                  O'tgan oylik tolov holati
                </td>
                <td className={`py-2 px-6 ${paymentStatusColor}`}>
                  {paymentStatus}
                </td>
              </tr>
            </tbody>
          ...
          {paymentHistory.length > 0 ? (
            <table className="min-w-full table-auto border-collapse mt-4 shadow-lg rounded-lg overflow-hidden">
              <thead className="bg-gray-200">
                <tr>
                  <th className="py-2 px-6 text-left text-gray-700">
                    Yig'uvchi
                  </th>
              ...
              <tbody>
                {paymentHistory.map((payment) => (
                  <tr key={payment.id} className="border-b">
                    ...
                    <td className="py-2 px-6 text-gray-900">
                      {Number(payment.payment_amount).toLocaleString()} so'm
                    </td>
                    <td className="py-2 px-6 text-gray-900">
